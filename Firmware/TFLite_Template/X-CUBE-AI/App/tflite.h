...
#ifndef __AI_TFLITE_H__
#define __AI_TFLITE_H__
#pragma once

#include "ai_platform.h"

#define AI_TFLITE_MODEL_NAME          "tflite"

#define AI_TFLITE_IN_NUM       (1)
#define AI_TFLITE_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_S8, 32, 32, 1, 1, NULL), \
}
#define AI_TFLITE_IN_SIZE { \
  (32 * 32 * 1), \
}
#define AI_TFLITE_IN_1_SIZE  (32 * 32 * 1)
#define AI_TFLITE_IN_1_SIZE_BYTES  ((32 * 32 * 1) * 1)




#define AI_TFLITE_OUT_NUM      (1)
#define AI_TFLITE_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_S8, 1, 1, 43, 1, NULL), \
}
#define AI_TFLITE_OUT_SIZE { \
  (1 * 1 * 43), \
}
#define AI_TFLITE_OUT_1_SIZE  (1 * 1 * 43)
#define AI_TFLITE_OUT_1_SIZE_BYTES  ((1 * 1 * 43) * 1)
 ...
 * @ref ai_tflite_create. Both the activations memory buffer 
 ...
 * header generated by the code generation tool. See AI_TFLITE_*
 * defines into file @ref tflite.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_tflite_forward(
  ai_handle network, const ai_buffer* input);


AI_API_DECLARE_END

#endif /*__AI_TFLITE_H__*/
