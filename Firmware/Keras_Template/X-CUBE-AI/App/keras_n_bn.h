...
#ifndef __AI_KERAS_N_BN_H__
#define __AI_KERAS_N_BN_H__
#pragma once

#include "ai_platform.h"

#define AI_KERAS_N_BN_MODEL_NAME          "keras_n_bn"

#define AI_KERAS_N_BN_IN_NUM       (1)
#define AI_KERAS_N_BN_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 32, 32, 1, 1, NULL), \
}
#define AI_KERAS_N_BN_IN_SIZE { \
  (32 * 32 * 1), \
}
#define AI_KERAS_N_BN_IN_1_SIZE  (32 * 32 * 1)
#define AI_KERAS_N_BN_IN_1_SIZE_BYTES  ((32 * 32 * 1) * 4)




#define AI_KERAS_N_BN_OUT_NUM      (1)
#define AI_KERAS_N_BN_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_FLOAT, 1, 1, 43, 1, NULL), \
}
#define AI_KERAS_N_BN_OUT_SIZE { \
  (1 * 1 * 43), \
}
#define AI_KERAS_N_BN_OUT_1_SIZE  (1 * 1 * 43)
#define AI_KERAS_N_BN_OUT_1_SIZE_BYTES  ((1 * 1 * 43) * 4)
 ...
 * @ref ai_keras_n_bn_create. Both the activations memory buffer 
 ...
 * using @ref ai_keras_n_bn_get_error
 */
AI_API_ENTRY
 ...
 * header generated by the code generation tool. See AI_KERAS_N_BN_*
 * defines into file @ref keras_n_bn.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
 ...
 */
AI_API_ENTRY
ai_i32 ai_keras_n_bn_forward(
  ai_handle network, const ai_buffer* input);


AI_API_DECLARE_END

#endif /*__AI_KERAS_N_BN_H__*/
